            ...
            <section>
              ...
              <div className="bg-white rounded-[14px] border border-divider-soft divide-y divide-[#f0f0f0]">
                <div className="p-4 sm:p-5">
                  <label className="block text-[11px] font-medium text-[#86868b] mb-1.5">{t("settings.dormitoryName")}</label>
                  <input value={dormName} onChange={(e) => setDormName(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-md border border-hairline text-[13px] text-ink focus:outline-none focus:ring-2 focus:ring-primary" />
                </div>
                <div className="p-4 sm:p-5">
                  <label className="block text-[11px] font-medium text-[#86868b] mb-1.5">{t("settings.dormitoryAddress")}</label>
                  <textarea value={address} onChange={(e) => setAddress(e.target.value)} rows={2}
                    className="w-full px-4 py-2.5 rounded-md border border-hairline text-[13px] text-ink focus:outline-none focus:ring-2 focus:ring-primary resize-none" />
                </div>
                <div className="p-4 sm:p-5">
                  <label className="block text-[11px] font-medium text-[#86868b] mb-1.5">{t("settings.phone")}</label>
              ...
              <div className="bg-white rounded-[14px] border border-divider-soft p-2">
                <div className="flex gap-1">
                  <button onClick={() => switchLocale("th")} disabled={isPending}
                    className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-md text-[13px] font-medium transition-all ${locale === "th" ? "bg-primary text-white shadow-sm" : "text-[#86868b] hover:text-ink hover:bg-canvas-parchment"}`}>
                    🇹🇭 {t("settings.thai")}
                  </button>
                  <button onClick={() => switchLocale("en")} disabled={isPending}
                    className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-md text-[13px] font-medium transition-all ${locale === "en" ? "bg-primary text-white shadow-sm" : "text-[#86868b] hover:text-ink hover:bg-canvas-parchment"}`}>
                    🇺🇸 {t("settings.english")}
                  </button>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                      ...
                      <input value={bankName} onChange={(e) => setBankName(e.target.value)}
                        className="w-full px-3 py-2 rounded-md border border-hairline text-[13px] text-ink focus:outline-none focus:ring-2 focus:ring-primary" />
                    </div>
                    <div>
        ...
        {activeTab === "rates" && (
          <div className="space-y-6">
              ...
              <div className="p-4 sm:p-5 flex items-center gap-4">
                ...
                <div className="relative w-28">
                  <input type="number" step="0.5" min="0" value={waterRate} onChange={(e) => setWaterRate(parseFloat(e.target.value) || 0)}
                    className="w-full pl-3 pr-8 py-2 rounded-md border border-hairline text-[13px] font-semibold text-right focus:outline-none focus:ring-2 focus:ring-primary" />
              ...
              <div className="p-4 sm:p-5 flex items-center gap-4">
                ...
                <div className="relative w-28">
                  <input type="number" step="0.5" min="0" value={electricRate} onChange={(e) => setElectricRate(parseFloat(e.target.value) || 0)}
                    className="w-full pl-3 pr-8 py-2 rounded-md border border-hairline text-[13px] font-semibold text-right focus:outline-none focus:ring-2 focus:ring-[#d97706]" />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-[#86868b]">฿</span>
                </div>
              </div>
              <div className="p-4 sm:p-5 flex items-center gap-4">
                <div className="w-9 h-9 rounded-[10px] bg-canvas-parchment flex items-center justify-center"><Building2 className="w-4 h-4 text-[#86868b]" /></div>
                <div className="flex-1 min-w-0">
            ...
            <div className="bg-white rounded-[14px] border border-divider-soft p-4 sm:p-5">
              ...
              <div className="py-2.5 flex items-center justify-between text-[13px] border-b border-canvas-parchment">
                <span className="flex items-center gap-2 text-[#86868b]"><Droplets className="w-3.5 h-3.5 text-primary" /> {t("rates.water")} (10 m³)</span>
                <span className="font-medium text-ink">{formatCurrency(preview.water)}</span>
              </div>
              <div className="py-2.5 flex items-center justify-between text-[13px] border-b border-canvas-parchment">
                <span className="flex items-center gap-2 text-[#86868b]"><Zap className="w-3.5 h-3.5 text-[#d97706]" /> {t("rates.electric")} (100 kWh)</span>
                <span className="font-medium text-ink">{formatCurrency(preview.electric)}</span>
              </div>
              <div className="py-2.5 flex items-center justify-between text-[13px] border-b border-canvas-parchment">
                <span className="flex items-center gap-2 text-[#86868b]"><Building2 className="w-3.5 h-3.5 text-[#86868b]" /> {t("rates.service")}</span>
                <span className="font-medium text-ink">{formatCurrency(preview.service)}</span>
              </div>
              <div className="py-2.5 flex items-center justify-between text-[13px] border-b border-canvas-parchment">
                <span className="flex items-center gap-2 text-[#86868b]"><span className="w-3.5 h-3.5 text-primary font-bold text-[9px] flex items-center justify-center">WiFi</span> {t("settings.wifiRate")}</span>
                <span className="font-medium text-ink">{formatCurrency(preview.wifi)}</span>
              </div>
              <div className="pt-3 flex items-center justify-between">
                <span className="text-[15px] font-semibold text-ink">{t("invoices.printTotalDue")}</span>
                <span className="text-[17px] font-semibold text-primary">{formatCurrency(4500 + preview.total)}</span>
              </div>
            </div>
